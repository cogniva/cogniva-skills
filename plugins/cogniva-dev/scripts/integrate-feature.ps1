...
try {
    ...
    # 1. Pre-merge target into the feature inside the isolated worktree. This
    #    deliberately permits a clean merge commit so the later local push is
    #    still a fast-forward. A conflicted attempt is aborted: neither branch
    #    is left changed and no integration push is attempted.
