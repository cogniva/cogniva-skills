...
Invoke: `/cogniva-skills:module-status <Module>`.
...
1. **Feature folders** (contain `<Feature>-plan.md`): do the same scan as
   `/cogniva-skills:feature-status`, scoped to this Module —
...
`/cogniva-skills:backlog`; to start one, `/cogniva-skills:plan-feature` (stub) or
`/cogniva-skills:execute-feature` (planned).
