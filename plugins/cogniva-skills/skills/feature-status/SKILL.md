...
1. Find feature folders: `docs/plans/<Module>/*/` (or `docs/plans/*/*/` for all).
   Each holds `<Feature>-plan.md` and usually `state.md`.
2. For each feature, from the plan compute task progress by counting task
...
3. From `state.md`, read `Target branch`, `Worktree`, and `Integration:` status
   (e.g. `queued` means it finished but is waiting for a clean target tree).
...
```
<Module>
  <Feature>   <done>/<total>   next: Task <N> "<title>"   [⛔ gate at Task <G>]   [worktree: <path>]   [integration: queued]
  ...
